...
notcars = glob.glob('./nv/*.png')

print("Cars: Total count    = ", len(cars))
print("NonCars: Total count = ", len(notcars))
...
def image_pipeline(image):
    ...
    add_to_box_tracking(hot_windows)
...
def video_pipeline(image):
    ...
    global false_positive_threshold

    # Apply threshold to help remove false positives
    heat = apply_threshold(heat, false_positive_threshold)
...
def add_to_box_tracking(hot_windows):
    global box_list_tracking
    global tracking_count

    if len(hot_windows) == 0:
        return

    if len(box_list_tracking) == tracking_count:
        del box_list_tracking[0]

    box_list_tracking.append(hot_windows)

def get_frame_boxes():
    box_list = []
    for l in box_list_tracking:
        box_list += l
    return box_list
